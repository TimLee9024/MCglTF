...
public class MCglTF {
	...
	public static void onEvent(FMLClientSetupEvent event) {
		event.getMinecraftSupplier().get().execute(() -> {
			...
			default:
				...
				if(glCapabilities.OpenGL43) INSTANCE.createSkinningProgramGL43();
				else if(glCapabilities.OpenGL33) INSTANCE.createSkinningProgramGL33();
				break;
			}
	...
	public static void onEvent(ModelBakeEvent event) {
		...
		case GL30:
			...
			break;
		case GL20:
			INSTANCE.processRenderedGltfModelsGL20(lookup);
			break;
		default:
			GLCapabilities glCapabilities = GL.getCapabilities();
			if(glCapabilities.OpenGL43) INSTANCE.processRenderedGltfModelsGL43(lookup);
			else if(glCapabilities.OpenGL40) INSTANCE.processRenderedGltfModelsGL40(lookup);
			else if(glCapabilities.OpenGL33) INSTANCE.processRenderedGltfModelsGL33(lookup);
			else INSTANCE.processRenderedGltfModelsGL20(lookup);
			break;
		}
