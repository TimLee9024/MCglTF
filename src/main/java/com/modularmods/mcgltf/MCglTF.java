...
import com.modularmods.mcgltf.iris.RenderedGltfModelGL20Iris;
...
public class MCglTF implements ModInitializer {
	...
	public void onInitialize() {
		...
		if(FabricLoader.getInstance().isModLoaded("iris")) {
			...
			processRenderedGltfModelSelector = (lookup) -> {
				...
				case GL30:
					...
					break;
				case GL20:
					processRenderedGltfModelsGL20Iris(lookup);
					break;
				default:
					GLCapabilities glCapabilities = GL.getCapabilities();
					if(glCapabilities.OpenGL43) processRenderedGltfModelsGL43Iris(lookup);
					else if(glCapabilities.OpenGL40) processRenderedGltfModelsGL40Iris(lookup);
					else if(glCapabilities.OpenGL33) processRenderedGltfModelsGL33Iris(lookup);
					else processRenderedGltfModelsGL20Iris(lookup);
					break;
				}
			};
		}
			...
			if(FabricLoader.getInstance().isModLoaded("optifabric")) {
				shaderModActive = () -> net.optifine.shaders.Shaders.isShaderPackInitialized;
			}
			else {
			...
			processRenderedGltfModelSelector = (lookup) -> {
				...
				case GL30:
					...
					break;
				case GL20:
					processRenderedGltfModelsGL20(lookup);
					break;
				default:
					GLCapabilities glCapabilities = GL.getCapabilities();
					if(glCapabilities.OpenGL43) processRenderedGltfModelsGL43(lookup);
					else if(glCapabilities.OpenGL40) processRenderedGltfModelsGL40(lookup);
					else if(glCapabilities.OpenGL33) processRenderedGltfModelsGL33(lookup);
					else processRenderedGltfModelsGL20(lookup);
					break;
				}
			};
		}
		...
		Minecraft.getInstance().execute(() -> {
			...
			default:
				GLCapabilities glCapabilities = GL.getCapabilities();
				if(glCapabilities.OpenGL43) createSkinningProgramGL43();
				else if(glCapabilities.OpenGL33) createSkinningProgramGL33();
				break;
			}
	...
	private void processRenderedGltfModelsGL20Iris(Map<ResourceLocation, MutablePair<GltfModel, List<IGltfModelReceiver>>> lookup) {
		processRenderedGltfModels(lookup, RenderedGltfModelGL20Iris::new);
	}
	...
	private String provider(String filename) {
		return "#Set maximum version of OpenGL to enable some optimizations for rendering glTF model.\n"
				...
				+ "#Allowed Values: AUTO, GL43, GL40, GL33, GL30, GL20\n"
